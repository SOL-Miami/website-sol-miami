...
// Define Directory
define( 'STATIC_BLOCK_DIR', plugin_dir_path( __FILE__ ) );
	...
	static function init_static_blocks() {
		if (function_exists('register_post_type')) {
			register_post_type('static_block',
				array(
					'labels' => array(
							'name' =>				_x('Static Content Blocks', 'post type general name', 'bethlehem-extension'),
							'singular_name' =>		_x('Static Block', 'post type singular name', 'bethlehem-extension'),
							'add_new' =>			_x('Add New', 'block', 'bethlehem-extension'),
							'add_new_item' =>		__('Add New Block', 'bethlehem-extension'),
							'edit_item' =>			__('Edit Block', 'bethlehem-extension'),
							'new_item' =>			__('New Block', 'bethlehem-extension'),
							'all_items' =>			__('Static Blocks', 'bethlehem-extension'),
							'view_item' =>			__('View Block', 'bethlehem-extension'),
							'search_items' =>		__('Search', 'bethlehem-extension'),
							'not_found' =>			__('No blocks found', 'bethlehem-extension'),
							'not_found_in_trash' =>	__('No blocks found in Trash', 'bethlehem-extension'), 
							'parent_item_colon' => '',
...
$meta_box_static_blocks = array(
	...
	'title' =>  __('Content Options', 'bethlehem-extension'),
	'page' => 'static_block',
	'context' => 'side',
	'priority' => 'default',
	'fields' => array(
    	array(
    	   'name' => __('Content Filters', 'bethlehem-extension'),
    	   'desc' => __('Apply all WP content filters? This will include plugin added filters.', 'bethlehem-extension'),
    	   'id' => 'content_filters',
    	   'type' => 'radio',
    	   'std' => '',
    	   'options' => array(
    	   		'default' => __('Defaults (recommended)', 'bethlehem-extension'),
    	   		'all' => __('All Content Filters', 'bethlehem-extension')
    	   	)
    	),
    	array(
    	   'name' => __('Auto Paragraphs', 'bethlehem-extension'),
    	   'desc' => __('Add &lt;p&gt; and &lt;br&gt; tags automatically.<br>(disabling may fix layout issues)', 'bethlehem-extension'),
    	   'id' => 'wpautop',
    	   'type' => 'radio',
    	   'std' => '',
    	   'options' => array(
    	   		'on' => __('On', 'bethlehem-extension'),
    	   		'off' => __('Off', 'bethlehem-extension')
    	   	)
    	)
   	)
);
...
function bethlehem_add_box_static_blocks() {
	global $meta_box_static_blocks;
	
	add_meta_box($meta_box_static_blocks['id'], $meta_box_static_blocks['title'], 'bethlehem_show_box_static_blocks', $meta_box_static_blocks['page'], $meta_box_static_blocks['context'], $meta_box_static_blocks['priority']);

}

add_action('admin_menu', 'bethlehem_add_box_static_blocks');
...
function bethlehem_show_box_static_blocks() {
	global $meta_box_static_blocks, $post;
 	
	// Use nonce for verification
	echo '<input type="hidden" name="bethlehem_meta_box_nonce" value="', wp_create_nonce(plugin_basename(__FILE__)), '" />';
...
add_action('save_post', 'bethlehem_save_data_static_blocks');
...
function bethlehem_save_data_static_blocks($post_id) {
	global $meta_box_static_blocks;
 
	// verify nonce
	if ( !isset($_POST['bethlehem_meta_box_nonce']) || !wp_verify_nonce($_POST['bethlehem_meta_box_nonce'], plugin_basename(__FILE__))) {
		return $post_id;
	}
