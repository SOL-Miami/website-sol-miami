...
if( ! class_exists('Bethlehem_Widget_Recent_Posts') ) {
	class Bethlehem_Widget_Recent_Posts extends WP_Widget {

		/**
		 * Sets up the widgets name etc
		 */
		public function __construct() {
			$this->widget_cssclass    = 'bethlehem_widget_recent_entries';
			$this->widget_description = __( 'Display Recent Posts.', 'bethlehem' );
			$this->widget_id          = 'bethlehem-recent-posts';
			$this->widget_name        = __( 'Bethlehem Recent Posts', 'bethlehem' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'std'   => __( 'Recent Posts', 'bethlehem' ),
					'label' => __( 'Title for widget', 'bethlehem' )
				),
				'number'  => array(
					'type'  => 'text',
					'std'   => '',
					'label' => __( 'Limit', 'bethlehem' )
				),
				'type'	=> array(
					'type'		=> 'select',
					'std'  		=> 'sidebar',
					'options'	=> array(
		                    'sidebar' => __( 'Sidebar', 'bethlehem' ),
		                    'type-1'  => __( 'Type 1', 'bethlehem' ),
		                    'type-2'  => __( 'Type 2', 'bethlehem' ),
		                    'type-3'  => __( 'Type 3', 'bethlehem' )
	                    ),
					'label'		=> __( 'Type', 'bethlehem' )
				),
				'show_date'	=> array(
					'type'		=> 'checkbox',
					'std'   	=> '',
					'label'		=> __( 'Display post date?', 'bethlehem' )
				)
			);
		...
		public function widget($args, $instance) {
			$cache = array();
			if ( ! $this->is_preview() ) {
				$cache = wp_cache_get( 'bethlehem-recent-posts', 'widget' );
			}
			...
			bethlehem_get_template( 'widgets/recent-posts-widget.php', array( 'args' => $args,'instance' => $instance ) );
