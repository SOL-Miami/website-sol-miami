 ...
 * @since 1.0.0
 */
if( ! class_exists('Bethlehem_Custom_Post_Type_Widgets_Archives') ) {
	class Bethlehem_Custom_Post_Type_Widgets_Archives extends WP_Widget {

		public function __construct() {
			$widget_ops = array( 'classname' => 'widget_archive', 'description' => __( 'A monthly archive of your site&#8217;s Posts.', 'bethlehem' ) );
			parent::__construct( 'custom-post-type-archives', __( 'Archives (Custom Post Type)', 'bethlehem' ), $widget_ops );
		...
		public function widget( $args, $instance ) {
			...
			$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( 'Archives', 'bethlehem' ) : $instance['title'], $instance, $this->id_base );
	...
	?>
				<select name="archive-dropdown" onchange='document.location.href=this.options[this.selectedIndex].value;'>
					<option value=""><?php echo esc_attr( __( 'Select Month', 'bethlehem' ) ); ?></option>
					<?php
	...
	?>
			...
			<p><input class="checkbox" type="checkbox" <?php echo esc_attr( $dropdown ); ?> id="<?php echo esc_attr( $this->get_field_id( 'dropdown' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'dropdown' ) ); ?>" /> <label for="<?php echo esc_attr( $this->get_field_id( 'dropdown' ) ); ?>"><?php _e( 'Display as dropdown', 'bethlehem' ); ?></label><br>
			<input class="checkbox" type="checkbox" <?php echo esc_attr( $count ); ?> id="<?php echo esc_attr( $this->get_field_id( 'count' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'count' ) ); ?>" /> <label for="<?php echo esc_attr( $this->get_field_id( 'count' ) ); ?>"><?php _e( 'Show post counts' , 'bethlehem' ); ?></label></p>
	<?php
		}
			...
			if ( 'monthly' == $r['type'] ) {
				...
				if ( $results ) {
					...
					foreach ( (array) $results as $result ) {
						...
						$text = sprintf( __( '%1$s %2$d', 'bethlehem' ), $wp_locale->get_month( $result->month ), $result->year );
